...
from utils_train import online_draw
import copy
import time
import cv2
...
def main():
    ...
    init_action_dataset = InitActionDataset(init_actor_batch = init_actor_batch)
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for train_step in range(1, 2500000):
            # startTime = time.time()
            # getting a seq of training data
            frame_name_list, gt, length = train_dataset.next()

            gau_frame = frame_name_list[0]  # the gaussian frame
            primitive_car_id = gau_frame[gau_frame.find('image_2') + 8 : gau_frame.find('image_2') + 16]
            # print "primitive_car_id", primitive_car_id
            ...
            # generate (64 * 107 *107 *3) pairs with distance
            
            for init_num in range(1):
                actor_samples, distance = init_action_dataset.next(primitive_car_id)    #the action need to move to the next pic
                batch_l = np.zeros([init_actor_batch, 107, 107, 3])
                for frame in range(init_actor_batch):
                    batch_l[frame] = skimage.io.imread(actor_samples[frame])
                distance[distance > 1] = 1
                distance[distance < -1] = -1
                batch_g = copy.deepcopy(batch_l)
            ...
            for frame in range(1, length):
                ...
                if np.random.random(1) < var:
                    deta_pos_ = gt[frame] - pos
                    if np.max(abs(deta_pos_)) < 1:
                        deta_pos = np.array([0,0,0])

                # pos_ = move_crop(pos_, deta_pos, img_size, rate)
                pos_ = pos + deta_pos 

                img_crop_l_, img_crop_g_ = draw_new_pos_image(frame_name_list[frame], pos_)
...
def draw_new_pos_image(addr, pos):
    '''
    draw the image with given pos
    '''
    image_id = addr[addr.find('image_2') + 8 : addr.find('image_2') + 14]
    car_id = int(addr[addr.find('image_2') + 15])
    img_tmp = online_draw(image_id,car_id, pos, 'train')
    return img_tmp, copy.deepcopy(img_tmp)
