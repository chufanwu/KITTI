...
class ILSVRCDataset(object):
    def __init__(self, data_path, data_home):
        ...
        for i in range(self.n_seq):
            self.seq_names.append(self.dataset[i]['seq_name'])  # the address of the image
        for i in range(self.n_seq):
            self.gt.append(np.array(self.dataset[i]['gt']))     # ground truth 2D bbox
        for i in range(self.n_seq):
            self.ranges.append((self.dataset[i]['start_frame'], self.dataset[i]['end_frame']))  # the length of the seq
            # the length is 10 
        self.lookahead = 20
