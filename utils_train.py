...
def online_draw(image_id, car_id, pos, attri = 'train'):
    ...
    for line in open(box2d_file).readlines():   # for every object
        ...
        if obj['name'] == 'Car' and truncated<0.1 and occluded<=1 and xy_diff >=25:
            ...
            if count_car == car_id:
                ...
                sam_2D = project3d2d(Corners, R, obj, Rt)
                d3ymin, d3ymax, d3xmin, d3xmax = get_boudary(sam_2D, img.shape)
                ...
                if(max(x_tmp,y_tmp)>p_size_para[1]):
                    ...
                    p_size = 1
                d3patch_draw = draw3d(sam_2D, d3patch, (0, 255, 0), p_size = p_size, crop_shift = [d3xmin,d3ymin])
...
def write2file(img, attri, image_id, label_xyz_tuple, i, j, car_id=0):  #i j means the 256 * 10 
    ...
    with open(out_label_dir,"w+") as afile:
        afile.write("%s\n"%(out_label_str))
...
def generate_all_data():
    ...
    for attriItem in attri_list[0:1]:    # only generate training and validation
